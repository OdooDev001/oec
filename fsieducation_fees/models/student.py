...
class OpStudentFeesDetails(models.Model):
    _name = "fsi.student.fees.details"
    _description = "Student Fees Details"
    _rec_name = 'student_id'

    fees_line_id = fields.Many2one('fsi.fees.terms.line', 'Fees Line')
    ...
    course_id = fields.Many2one('fsi.course', 'Course', required=False)
    batch_id = fields.Many2one('fsi.batch', 'Batch', required=False)
    ...
    def get_invoice(self):
        ...
        if self.amount <= 0.00:
            ...
            amount = self.amount
            name = product.name
        element_id = self.env['fsi.fees.element'].search([
...
class OpStudent(models.Model):
    ...
    fees_detail_ids = fields.One2many('fsi.student.fees.details',
                                      'student_id',
    ...
    def _compute_fees_details(self):
        for fees in self:
            fees.fees_details_count = self.env['fsi.student.fees.details'].search_count(
                [('student_id', '=', self.id)])
