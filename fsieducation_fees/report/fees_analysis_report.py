...
class ReportFeesAnalysis(models.AbstractModel):
    _name = "report.fsieducation_fees.report_fees_analysis"
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if data['fees_filter'] == 'student':
            student_ids = self.env['fsi.student'].browse([data['student']])
        else:
            student_ids = self.env['fsi.student'].search(
                [('course_detail_ids.course_id', '=', data['course'])])
            course_id = self.env['fsi.course'].search([('id', '=', data['course'])])
            report_type = 'course'
            docargs.update({
