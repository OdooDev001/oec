...
class TestStudentFees(TestFeesCommon):
    ...
    def test_case_fees(self):
        fees = self.fsi_student_fees.search([])
        if not fees:
...
class TestStudent(TestFeesCommon):
    ...
    def test_case_student_fees(self):
        student = self.fsi_student.search([])
        if not student:
            raise AssertionError(
...
class TestWizardFees(TestFeesCommon):
    ...
    def test_case_wizard_fees(self):
        wizard = self.fsi_fees_wizard.create({
            'fees_filter': 'student',
...
class TestFeesTerms(TestFeesCommon):
    ...
    def test_case_fees_terms(self):
        terms = self.fsi_fees_terms.create({
            'name': 'Library Fees',
            'line_ids': self.env.ref('fsieducation_fees.fsi_fees_term_line_6'),
        })
