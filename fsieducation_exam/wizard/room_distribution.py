...
class OpRoomDistribution(models.TransientModel):
    ...
    room_ids = fields.Many2many("fsi.exam.room", string="Exam Rooms")
    student_ids = fields.Many2many("fsi.student", string='Student')
    ...
    def default_get(self, fields):
        ...
        reg_ids = self.env['fsi.subject.registration'].search(
    ...
    def schedule_exam(self):
        attendance = self.env['fsi.exam.attendees']
        if not self.room_ids or not self.student_ids:
            raise ValidationError(
                    _("Please Enter both Room And student"))

        booked_rooms = self.env['fsi.exam.attendees'].search([
        ...
        conflicting_students = self.env['fsi.exam.attendees'].search([
