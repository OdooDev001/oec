...
class FSIResultTemplate(models.Model):
    _name = "fsi.result.template"
    _inherit = ["mail.thread"]
    _description = "Result Template"

    exam_session_id = fields.Many2one(
        'fsi.exam.session', 'Exam Session',
    ...
    grade_ids = fields.Many2many(
        'fsi.grade.configuration', string='Grade Configuration')
    ...
    def generate_result(self):
        for record in self:
            marksheet_reg_id = self.env['fsi.marksheet.register'].create({
            ...
            for exam in record.exam_session_id.exam_ids:
                for attendee in exam.attendees_line:
                    result_line_id = self.env['fsi.result.line'].create({
            ...
            for student in student_dict:
                marksheet_line_id = self.env['fsi.marksheet.line'].create({
                    'student_id': student,
