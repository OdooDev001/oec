...
class GenerateSession(models.TransientModel):
    ...
    def act_gen_time_table(self):
        session_obj = self.env['fsi.session']
        data = []
        for session in self:
...
class GenerateSessionLine(models.TransientModel):
    ...
    faculty_id = fields.Many2one('fsi.faculty', 'Faculty', required=True)
    subject_id = fields.Many2one('fsi.subject', 'Subject', required=True)
    timing_id = fields.Many2one('fsi.timing', 'Timing')
    session_start_time = fields.Float("Start Time")
    session_end_time = fields.Float("End Time")
    classroom_id = fields.Many2one('fsi.classroom', 'Classroom')
