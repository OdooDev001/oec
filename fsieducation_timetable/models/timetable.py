...
class OpSession(models.Model):
    ...
    timing_id = fields.Many2one(
        'fsi.timing', 'Timing', tracking=True)
    ...
    faculty_id = fields.Many2one(
        'fsi.faculty', 'Faculty', required=True)
    batch_id = fields.Many2one(
        'fsi.batch', 'Batch', required=True)
    subject_id = fields.Many2one(
        'fsi.subject', 'Subject', required=True)
    classroom_id = fields.Many2one(
        'fsi.classroom', 'Classroom')
    ...
    def _compute_batch_users(self):
        student_obj = self.env['fsi.student']
    ...
    def check_timetable_fields(self):
        ...
        sessions = self.env['fsi.session'].search([])
    ...
    def create(self, values):
        ...
        for record in records:
            ...
            if record.batch_id and record.course_id:
                course_val = self.env['fsi.student.course'].search([
    ...
    def notify_user(self):
        for session in self:
            template = self.env.ref(
                'fsieducation_timetable.session_details_changes',
    ...
    def get_import_templates(self):
        return [{
            ...
            'template': '/fsieducation_timetable/static/xls/fsi_session.xls'
        }]
