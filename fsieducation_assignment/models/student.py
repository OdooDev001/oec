...
class FSIStudent(models.Model):
    ...
    allocation_ids = fields.Many2many('fsi.assignment', string='Assignment(s)')
    assignment_count = fields.Integer(compute='compute_count_assignment')

    def get_assignment(self):
        action = self.env.ref('fsieducation_assignment.'
                              'act_open_fsi_assignment_view').sudo().read()[0]
        action['domain'] = [('allocation_ids', 'in', self.ids)]
        return action

    def compute_count_assignment(self):
        for record in self:
            record.assignment_count = self.env['fsi.assignment'].search_count(
