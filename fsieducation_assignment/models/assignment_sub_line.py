...
class FSIAssignmentSubLine(models.Model):
    _name = "fsi.assignment.sub.line"
    ...
    def _compute_get_user_group(self):
        for user in self:
            if self.env.user.has_group(
                    'fsieducation_assignment.group_fsi_assignment_manager') or \
                    self.env.user.has_group(
                        'fsieducation_assignment.group_fsi_assignment_user'):
                user.user_boolean = True
            else:
                user.user_boolean = False

    assignment_id = fields.Many2one(
        'fsi.assignment', 'Assignment', required=True)
    student_id = fields.Many2one(
        'fsi.student', 'Student', required=True)
