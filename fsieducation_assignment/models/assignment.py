...
class GradingAssigment(models.Model):
    ...
    course_id = fields.Many2one('fsi.course', 'Course', required=True)
    subject_id = fields.Many2one('fsi.subject', string='Subject')
    ...
    faculty_id = fields.Many2one(
        'fsi.faculty', 'Faculty', default=lambda self: self.env[
            'fsi.faculty'].search([('user_id', '=', self.env.uid)]),
        required=True)
    point = fields.Float('Points')


class OpAssignment(models.Model):
    _name = "fsi.assignment"
    _inherit = "mail.thread"
    _description = "Assignment"
    _order = "submission_date DESC"
    _inherits = {"grading.assignment": "grading_assignment_id"}

    batch_id = fields.Many2one('fsi.batch', 'Batch', required=True)
    ...
    submission_date = fields.Datetime('Submission Date', required=True,
                                      tracking=True)
    allocation_ids = fields.Many2many('fsi.student', string='Allocated To')
    assignment_sub_line = fields.One2many('fsi.assignment.sub.line',
                                          'assignment_id', 'Submission')
    reviewer = fields.Many2one('fsi.faculty', 'Reviewer')
    ...
    def onchange_course(self):
        self.batch_id = False
        if self.course_id:
            subject_ids = self.env['fsi.course'].search([
    ...
    def get_assignment_submissions(self):
        return {
            ...
            'view_mode': 'list,form',
            'res_model': 'fsi.assignment.sub.line',
