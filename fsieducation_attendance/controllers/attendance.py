...
class FSIAttendanceController(http.Controller):

    @http.route(['/fsieducation-attendance/take-attendance'], type='json',
                auth='none', methods=['POST'], csrf=False)
    def create_attendance_lines(self, **post):
        sheet_id = post.get('attendance_sheet_id', False)
        if sheet_id:
            attend_lines = request.env['fsi.attendance.line'].sudo()
            sheet = request.env['fsi.attendance.sheet'].sudo().browse(
                [sheet_id])
            all_student_search = request.env['fsi.student'].sudo().search(
