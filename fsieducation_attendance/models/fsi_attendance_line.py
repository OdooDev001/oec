...
class FSIAttendanceLine(models.Model):
    _name = "fsi.attendance.line"
    ...
    attendance_id = fields.Many2one(
        'fsi.attendance.sheet', 'Attendance Sheet', required=True,
        tracking=True, ondelete="cascade")
    student_id = fields.Many2one(
        'fsi.student', 'Student', required=True, tracking=True)
    ...
    attendance_type_id = fields.Many2one(
        'fsi.attendance.type', 'Attendance Type',
...
# Instead of these boolean field we can add selection
    @api.onchange('attendance_type_id')
    def onchange_attendance_type(self):
        """To change the value of other field based on attendance_type_id"""
    ...
    def onchange_present(self):
        """To change the value of other field based on present"""
        if self.present:
            self.late = False
            self.excused = False
            self.absent = False
    ...
    def onchange_absent(self):
        """To change the value of other field based on absent"""
        if self.absent:
            self.present = False
            self.late = False
            self.excused = False
    ...
    def onchange_excused(self):
        """To change the value of other field based on excused"""
        if self.excused:
            self.present = False
            self.late = False
            self.absent = False
    ...
    def onchange_late(self):
        """To change the value of other field based on late"""
        if self.late:
            self.present = False
            self.excused = False
            self.absent = False
