...
class FSIAttendanceSheet(models.Model):
    _name = "fsi.attendance.sheet"
    ...
    register_id = fields.Many2one(
        'fsi.attendance.register', 'Register', required=True,
        tracking=True)
    course_id = fields.Many2one(
        'fsi.course', related='register_id.course_id', store=True,
        readonly=True)
    batch_id = fields.Many2one(
        'fsi.batch', 'Batch', related='register_id.batch_id', store=True,
        readonly=True)
    session_id = fields.Many2one('fsi.session', 'Session')
    attendance_date = fields.Date(
        'Date', required=True, default=lambda self: fields.Date.today(),
        tracking=True)
    attendance_line = fields.One2many(
        'fsi.attendance.line', 'attendance_id', 'Attendance Line')
    faculty_id = fields.Many2one('fsi.faculty', 'Faculty')
    ...
    def create(self, vals_list):
        for vals in vals_list:
            sheet = self.env['ir.sequence'].next_by_code('fsi.attendance.sheet')
            register = self.env['fsi.attendance.register']. \
