...
class FSIStudent(models.Model):
    ...
    activity_log = fields.One2many('fsi.activity', 'student_id',
                                   string='Activity Log')
    activity_count = fields.Integer(compute='compute_count')

    def get_activity(self):
        action = self.env.ref('fsieducation_activity.'
                              'action_open_fsi_activity_view').sudo().read()[0]
        action['domain'] = [('student_id', 'in', self.ids)]
        return action

    def compute_count(self):
        for record in self:
            record.activity_count = self.env['fsi.activity'].search_count(
