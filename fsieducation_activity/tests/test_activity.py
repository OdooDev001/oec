...
class TestActivity(TestActivityCommon):
    ...
    def test_case_activity_1(self):
        activity = self.fsi_activity.search([])
        if not activity:
...
class TestActivityType(TestActivityCommon):
    ...
    def test_case_1_activity_type(self):
        activity_type = self.fsi_activity_type.search([])
...
class TestStudentMigrateWizard(TestActivityCommon):
    ...
    def test_case_1_student_migrate_wizard(self):
        student_migrate = self.fsi_student_migrate_wizard.create({
            'course_from_id': self.env.ref('fsieducation_core.fsi_course_2').id,
            'course_to_id': self.env.ref('fsieducation_core.fsi_course_3').id,
            'batch_id': self.env.ref('fsieducation_core.fsi_batch_2').id,
            'student_ids': self.env.ref('fsieducation_core.fsi_student_1'),
        })
        student_migrate1 = self.fsi_student_migrate_wizard.create({
            'course_from_id': self.env.ref('fsieducation_core.fsi_course_3').id,
            'course_to_id': self.env.ref('fsieducation_core.fsi_course_2').id,
            'batch_id': self.env.ref('fsieducation_core.fsi_batch_1').id,
            'student_ids': self.env.ref('fsieducation_core.fsi_student_2'),
        })
