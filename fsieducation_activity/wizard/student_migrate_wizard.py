...
class StudentMigrate(models.TransientModel):
    ...
    course_from_id = fields.Many2one('fsi.course', 'From Course', required=True)
    course_to_id = fields.Many2one('fsi.course', 'To Course')
    batch_id = fields.Many2one('fsi.batch', 'To Batch')
    optional_sub = fields.Boolean("Optional Subjects")
    student_ids = fields.Many2many(
        'fsi.student', string='Student(s)', required=True)
    ...
    def student_by_course(self):
        ...
        if self.course_from_id:
            lists = []
            student_ids = self.env['fsi.student.course'].search([
    ...
    def student_migrate_forward(self):
        act_type = self.env.ref('fsieducation_activity.fsi_activity_type_3')
        for record in self:
                ...
                if self.course_completed:
                    for course_update in student.course_detail_ids:
                        if course_update.course_id == record.course_from_id:
                            ...
                            self.env['fsi.activity'].create(activity_vals)
                else:
                    for course_update in student.course_detail_ids:
                        if course_update.course_id == self.course_from_id:
                            ...
                            student_course = self.env['fsi.student.course'].search(
                                [('student_id', '=', student.id),
