...
class TestMediaUnit(TestLibraryCommon):
    ...
    def test_case_media_unit(self):
        mediaunit = self.fsi_media_unit.create({
            'name': 'Troposhere In Detail unit-006',
            'barcode': 'MUB0000067',
            'media_id': self.env.ref('fsieducation_library.fsi_media_1').id,
        })
...
class TestMediaMovement(TestLibraryCommon):
    ...
    def test_case_media_movement(self):
        mediamovement = self.fsi_media_movement.search([])
...
class TestMediaPurchase(TestLibraryCommon):
    ...
    def test_case_media_purchase(self):
        mediapurchase = self.fsi_media_purchase.create({
            'name': 'Accounting Made Simple',
            'author': 'Mike Piper',
            'requested_id':
                self.env.ref('fsieducation_core.fsi_res_partner_1').id,
            'course_ids': self.env.ref('fsieducation_core.fsi_course_1').id,
            'subject_ids': self.env.ref('fsieducation_core.fsi_subject_1').id
        })
...
class TestMediaQueue(TestLibraryCommon):
    ...
    def test_case_media_queue(self):
        mediaqueue = self.fsi_media_queue.create({
            'name': 'QUE001',
            'media_id': self.env.ref('fsieducation_library.fsi_media_1').id,
...
class TestLibraryCardType(TestLibraryCommon):
    ...
    def test_case_library_card_type(self):
        cardtype = self.fsi_library_card_type.search([])
        if not cardtype:
            raise AssertionError(
...
class TestLibraryCard(TestLibraryCommon):
    ...
    def test_case_library_card(self):
        card = self.fsi_library_card.create({
            'partner_id': self.env.ref('fsieducation_core.fsi_res_partner_1').id,
            'number': 'LCB0000000018',
            'library_card_type_id':
                self.env.ref('fsieducation_library.fsi_library_card_type_1').id,
...
class TestWizardIssue(TestLibraryCommon):
    ...
    def test_case_wizard_issue(self):
        wizard = self.wizard_issue.create({
            'media_id': self.env.ref('fsieducation_library.fsi_media_1').id,
            'media_unit_id':
                self.env.ref('fsieducation_library.fsi_media_unit_3').id,
            'type': 'student',
            'library_card_id':
                self.env.ref('fsieducation_library.fsi_library_card_type_1').id,
...
class TestWizardReturn(TestLibraryCommon):
    ...
    def test_case_wizard_return(self):
        return_wizard = self.return_media.create({
            'media_id': self.env.ref('fsieducation_library.fsi_media_1').id,
            'media_unit_id':
                self.env.ref('fsieducation_library.fsi_media_unit_2').id,
        })
