...
class IssueMedia(models.TransientModel):
    ...
    media_id = fields.Many2one('fsi.media', 'Media', required=True)
    media_unit_id = fields.Many2one('fsi.media.unit', 'Media Unit',
                                    required=True)
    ...
    library_card_id = fields.Many2one(
        'fsi.library.card', 'Library Card', required=True)
    ...
    def check_max_issue(self, student_id, library_card_id):
        media_movement_search = self.env["fsi.media.movement"].search(
            [('library_card_id', '=', library_card_id),
             ('student_id', '=', student_id),
             ('state', '=', 'issue')])
        if len(media_movement_search) < self.env["fsi.library.card"].browse(
        ...
        for media in self:
            ...
            if media.check_max_issue(media.student_id.id,
                ...
                if media.media_unit_id.state and \
                    ...
                    self.env['fsi.media.movement'].create(media_movement_create)
