...
class ReturnMedia(models.TransientModel):
    ...
    media_id = fields.Many2one('fsi.media', 'Media', readonly=True)
    media_unit_id = fields.Many2one(
        'fsi.media.unit', 'Media Unit', readonly=True, required=True)
    ...
    def do_return(self):
        for media in self:
            if media.media_unit_id.state and \
                ...
                media_move_search = self.env['fsi.media.movement'].search(
