...
class OpFaculty(models.Model):
    _inherit = "fsi.faculty"

    library_card_id = fields.Many2one('fsi.library.card', 'Library Card')
    media_movement_lines = fields.One2many(
        'fsi.media.movement', 'faculty_id', 'Movements')
    ...
    def _compute_media_movement_lines(self):
        for media in self:
            media.media_movement_lines_count = \
                self.env['fsi.media.movement'].search_count(
                    [('faculty_id', '=', self.id)])
