...
class FSIAdmission(models.Model):
    ...
    course_id = fields.Many2one(
        'fsi.course', 'Course', required=True)
    batch_id = fields.Many2one(
        'fsi.batch', 'Batch', required=False)
    street = fields.Char(
        'Street', size=256)
    street2 = fields.Char(
        'Street2', size=256)
    ...
    register_id = fields.Many2one(
        'fsi.admission.register', 'Admission Register', required=True)
    partner_id = fields.Many2one('res.partner', 'Partner')
    is_student = fields.Boolean('Is Already Student')
    fees_term_id = fields.Many2one('fsi.fees.terms', 'Fees Term')
    ...
    course_ids = fields.Many2many('fsi.course', string='Courses', compute='compute_course_ids')
    ...
    def compute_course_ids(self):
        """to get course data based on admission registration """
        for data in self:
            if data.register_id:
    ...
    def onchange_student(self):
        """to get student details if he/she is already a student"""
    ...
    def onchange_register(self):
        """based on the register the program value will be updated"""
        if self.register_id:
    ...
    def _check_birthdate(self):
        """Checking validation condition in birth date"""
        for record in self:
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('application_number', 'New') == 'New':
                vals['application_number'] = self.env['ir.sequence'].next_by_code('fsi.admission') or 'New'
        return super().create(vals_list)

    def action_submit_form(self):
        """Button action to set in submit state"""
        self.state = 'submit'

    def action_confirm(self):
        """Button action to set in admission state"""
        self.state = 'admission'

    def action_in_progress(self):
        """Button action to set in progress state"""
        for record in self:
            record.state = 'confirm'

    def get_student_vals(self):
        """Get student values , student regarding details"""
        enable_create_student_user = self.env['ir.config_parameter'].get_param(
            'fsieducation_admission.enable_create_student_user')
        for student in self:
            student_user = False
    ...
    def action_enroll_student(self):
        """Action Enroll Student for enrolling student"""
        for record in self:
            if record.register_id.max_count:
                total_admission = self.env['fsi.admission'].search_count(
            ...
            reg_id = self.env['fsi.subject.registration'].create({
                'student_id': student_id,
    ...
    def action_cancel(self):
        """To Change the state to cancel"""
        self.state = 'cancel'
        if self.is_student and self.student_id.fees_detail_ids:
            self.student_id.fees_detail_ids.state = 'cancel'

    def payment_process(self):
        """To Change the state to fees_paid *** """
        self.state = 'fees_paid'

    def action_open_student(self):
        """Button to open student view"""
        form_view = self.env.ref('fsieducation_core.view_fsi_student_form')
        tree_view = self.env.ref('fsieducation_core.view_fsi_student_tree')
        value = {
            'domain': str([('id', '=', self.student_id.id)]),
            'view_type': 'form',
            'view_mode': 'list, form',
            'res_model': 'fsi.student',
            'view_id': False,
    ...
    def get_import_templates(self):
        ...
        return [{
            ...
            'template': '/fsieducation_admission/static/xls/fsi_admission.xls'
        }]
