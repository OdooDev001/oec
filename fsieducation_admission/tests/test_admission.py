...
class TestAdmission(TestAdmissionCommon):
    ...
    def test_case_1_admissions(self):
        admissions = self.fsi_admission.search([])
        self.fsi_admission._onchange_name()
        ...
        vals = {
            ...
            'application_number': self.env.ref(
                'fsieducation_admission.seq_fsi_admission').id,
            'birth_date': '2002-12-20',
            'course_id': self.env.ref('fsieducation_core.fsi_course_5').id,
            'batch_id': self.env.ref('fsieducation_core.fsi_batch_4').id,
            'phone': 32234234,
            'mobile': 93432,
            ...
            'register_id': self.env.ref(
                'fsieducation_admission.fsi_admission_register_3').id,
            'image': False
        }

        studnet_1 = self.fsi_admission.create(vals)
        ...
        vals_2 = {
            ...
            'register_id': self.env.ref(
                'fsieducation_admission.fsi_admission_register_3').id,
            'student_id': self.env.ref('fsieducation_core.fsi_student_18').id,
            'fees_term_id': 2,
            'fees': 1000,
        }

        studnet_2 = self.fsi_admission.create(vals_2)
...
class TestAdmissionregister(TestAdmissionCommon):
    ...
    def test_case_1_register(self):
        register = self.fsi_register.search([])
