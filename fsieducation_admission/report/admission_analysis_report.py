...
class ReportAdmissionAnalysis(models.AbstractModel):
    _name = "report.fsieducation_admission.report_admission_analysis"
    _description = "Admission Analysis Report"

    def get_total_student(self, data):
        student_search = self.env['fsi.admission'].search_count(
            [('state', '=', 'done'),
    ...
    def get_data(self, data):
        lst = []
        student_search = self.env['fsi.admission'].search(
            [('state', '=', 'done'),
             ('course_id', '=', data['course_id'][0]),
             ('admission_date', '>=', data['start_date']),
             ('admission_date', '<=', data['end_date'])],
