...
class FSIFaculty(models.Model):
    _name = "fsi.faculty"
    _description = "FSI Faculty"
    ...
    faculty_subject_ids = fields.Many2many('fsi.subject', string='Subject(s)',
                                           tracking=True)
    emp_id = fields.Many2one('hr.employee', 'HR Employee')
    main_department_id = fields.Many2one(
        'fsi.department', 'Main Department',
        default=lambda self:
        self.env.user.dept_id and self.env.user.dept_id.id or False)
    allowed_department_ids = fields.Many2many(
        'fsi.department', string='Allowed Department',
        default=lambda self:
    ...
    def get_import_templates(self):
        return [{
            ...
            'template': '/fsieducation_core/static/xls/fsi_faculty.xls'
        }]
