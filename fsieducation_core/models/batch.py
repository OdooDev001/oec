...
class FSIBatch(models.Model):
    _name = "fsi.batch"
    _inherit = "mail.thread"
    _description = "FSI Batch"
    ...
    def name_search(self, name, args=None, operator='ilike', limit=100):
        if self.env.context.get('get_parent_batch', False):
            ...
            courses = self.env['fsi.course'].browse(lst)
            while courses.parent_id:
                lst.append(courses.parent_id.id)
                courses = courses.parent_id
            batches = self.env['fsi.batch'].search([('course_id', 'in', lst)])
    ...
    def get_import_templates(self):
        return [{
            ...
            'template': '/fsieducation_core/static/xls/fsi_batch.xls'
        }]
