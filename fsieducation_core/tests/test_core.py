...
class TestCore(TestCoreCommon):
    ...
    def test_case_2_course(self):
        courses = self.fsi_course.search([])

        for course in courses:
            course.get_import_templates()

    def test_case_3_faculty(self):
        faculty = self.fsi_faculty.search([])

        new_faculty = self.fsi_faculty.create({
            'partner_id': self.env.ref('fsieducation_core.fsi_res_partner_30').id,
    ...
    def test_case_5_res_company(self):
        ...
        record = self.env.ref('fsieducation_core.fsi_faculty_1'),
        users.create_user(record)

    def test_case_6_student(self):
        students = self.fsi_student.search([])
        ...
        vals = {
            ...
            'gender': 'm',
            'partner_id': self.env.ref('fsieducation_core.fsi_res_partner_14').id

        }
        new_student = self.fsi_student.create(vals)
    ...
    def test_case_7_update(self):
        self.fsi_update._get_message()
        self.fsi_update.update_notification(cron_mode=True)
    ...
    def test_case_9_faculty_user_wizard(self):
        ...
        faculty_id = self.env.ref('fsieducation_core.fsi_faculty_1')
    ...
    def test_case_10_studnet_wizard(self):
        student_id = self.env.ref('fsieducation_core.fsi_student_1')
